...
void accessData (
    ...
    for (size_t line_index = 0; line_index < E; line_index++) {
        if (cache_set[line_index].valid && cache_set[line_index].tag == tag) {
            cache_set[line_index].recent_use = program_counter;
            (*hit_count)++;
            return;
        }
    }
    ...
    size_t min_recent_use = cache_set[0].recent_use;
    size_t lru_line_index = 0;
    for (size_t line_index = 1; line_index < E; line_index++) {
        if (cache_set[line_index].recent_use < min_recent_use) {
            min_recent_use = cache_set[line_index].recent_use;
            lru_line_index = line_index;
        }
    }

    // If cache set line already in use, then record an eviction
    if (cache_set[lru_line_index].valid) {
        (*eviction_count)++;
    }

    // Update the cache line with the new tag and recent use
    cache_set[lru_line_index].valid = true;
    cache_set[lru_line_index].tag = tag;
    cache_set[lru_line_index].recent_use = program_counter;
}
...
int main(int argc, char* argv[]) {
    ...
    sscanf(line_buf, " %c %llx,%d", &access_type, &addr, &size);

    if (access_type == 'L' || access_type == 'S' || access_type == 'M') {
        accessData(addr, cache, &hit_count, &miss_count, &eviction_count);

        // For 'M' (modify) access type, simulate a second access (store) after the first one (load)
        if (access_type == 'M') {
            accessData(addr, cache, &hit_count, &miss_count, &eviction_count);
        }
     }
