...
int main ( int argc, char* argv[] ) {
    ...
    for (unsigned iter=0; iter<graphNodeCount-1; iter++) {
        ...
        for (graphNode_t source=0; source<graphNodeCount; source++)
       {
            ...
            { // if already visited
                /* ... */
             AdjacencyListNode* node = &adjacencyList[source];
             while (node != NULL) 
             {
                if (parents[node->graphNode] == -1 && node->weight < minWeight) {
                    // we found a new minimum weight edge
    ...
    for (graphNode_t node=0; node<graphNodeCount; node++) {
        if (parents[node] != -1 && parents[node] != node) {
            printf("%ld %ld\n", node , parents[node]);
        }
    }
