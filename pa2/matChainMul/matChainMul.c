...
unsigned int cost (
    ...
    if ( matrixCount==1 ) { // Base case.
        return 0; // No multplication to bepwd done.
    } else {
...
int main(int argc, char* argv[]) {
    ...
    // unsigned int optimalMulOps = cost(matrixCount, rowSizes, colSizes);
    // printf("Optimal number of multiplications: %u\n", optimalMulOps);

    int** matMulProduct = calloc(rowSizes[0], sizeof(int*));
    for (unsigned int i = 0; i < rowSizes[0]; i++) {
        matMulProduct[i] = calloc(colSizes[matrixCount - 1], sizeof(int));
    }

    matMul(rowSizes[0], colSizes[0], colSizes[1], matrices[0], matrices[1], matMulProduct);

    // printf("Matrix Chain Product:\n");
    for (unsigned int i = 0; i < rowSizes[0]; i++) {
        for (unsigned int j = 0; j < colSizes[matrixCount - 1]; j++) {
            printf("%d ", matMulProduct[i][j]);
        }
        printf("\n");
    }
